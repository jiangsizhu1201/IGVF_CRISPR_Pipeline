...
convert_mudata_to_sceptre_object_v1 <- function(mudata, remove_collinear_covariates = FALSE){
  ...
  if(!is.null(SummarizedExperiment::colData(mudata))){
    ...
    factor_levels <- sapply(covariates, function(x) if (is.factor(x)) levels(x))
    single_level_factors <- sapply(factor_levels, function(x) length(x) < 2)
    covariates_clean <- covariates[, !single_level_factors, drop = FALSE]
...
mudata_out <- assign_grnas_sceptre_v1(mudata = mudata_in)

# write MuData
MuData::writeH5MU(mudata_out, file = 'sceptre_assignment_mudata.h5mu')